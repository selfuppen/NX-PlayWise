...
.PHONY: all manifest device-lab-manifest eden-test-manifest test-host test-python test companion-nro companion-overlay sysmodule-nsp eden-test-nro borealis-poc-nro borealis-poc-deko3d-nro borealis-poc-clean packages package-playwise package-complete device-lab-sysmodule device-lab-nro device-lab-overlay device-lab-package clean FORCE_HOST_REBUILD
...
borealis-poc-nro:
	$(STAGE_TIMER) borealis-poc configure -- cmake -S experiments/borealis_poc -B build/borealis-poc/cmake -DPLATFORM_SWITCH=ON -DUSE_DEKO3D=OFF -DUSE_SDL2=ON -DUSE_GLFW=OFF -DUSE_EGL=ON -DUSE_GL3=ON -DUSE_GL2=OFF -DUSE_GLES2=OFF -DUSE_GLES3=OFF -DUSE_SYSTEM_FMT=OFF -DUSE_SYSTEM_TINYXML2=OFF -DUSE_SYSTEM_TWEENY=OFF -DBRLS_UNITY_BUILD=OFF -DPLAYWISE_VERSION=$(PLAYWISE_VERSION) -DCMAKE_BUILD_TYPE=Release
...
borealis-poc-deko3d-nro:
	cmake -S experiments/borealis_poc -B build/borealis-poc-deko3d/cmake -DPLATFORM_SWITCH=ON -DUSE_DEKO3D=ON -DUSE_SDL2=OFF -DUSE_GLFW=OFF -DUSE_EGL=OFF -DUSE_GL3=OFF -DUSE_GL2=OFF -DUSE_GLES2=OFF -DUSE_GLES3=OFF -DUSE_SYSTEM_FMT=OFF -DUSE_SYSTEM_TINYXML2=OFF -DUSE_SYSTEM_TWEENY=OFF -DBRLS_UNITY_BUILD=OFF -DPLAYWISE_VERSION=$(PLAYWISE_VERSION) -DCMAKE_BUILD_TYPE=Release
	cmake --build build/borealis-poc-deko3d/cmake --target playwise-borealis-poc.nro --parallel 4
	mkdir -p build/borealis-poc-deko3d
	cp build/borealis-poc-deko3d/cmake/playwise-borealis-poc.nro build/borealis-poc-deko3d/playwise-borealis-poc-deko3d.nro

borealis-poc-clean:
	rm -rf build/borealis-poc build/borealis-poc-deko3d
...
clean:
	$(STAGE_TIMER) global clean -- sh -c 'rm -rf build/host build/generated build/switch build/packages build/device-lab && if [ "$(CLEAN_EDEN)" = "1" ]; then rm -rf build/eden-test; fi && if [ "$(CLEAN_BOREALIS_POC)" = "1" ]; then rm -rf build/borealis-poc build/borealis-poc-deko3d; fi && $(MAKE) -C companion/nro clean || true && rm -rf companion/nro/build-eden companion/nro/pctc-eden.elf companion/nro/pctc-eden.nro companion/nro/pctc-eden.nacp && $(MAKE) -C companion/overlay clean || true && $(MAKE) -C sysmodule clean || true && $(MAKE) -C device_lab/nro clean || true && $(MAKE) -C device_lab/overlay clean || true'
