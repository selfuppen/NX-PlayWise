...
Compose 默认把宿主端口 `1888` 映射到容器端口 `58791`，脚本以 `root@127.0.0.1:1888` 连接；映射已固定为 `127.0.0.1:1888:58791`。统一打包入口通过固定容器名 `devkitpro-ssh-v1` 执行 `docker inspect`，取得 Compose 配置的镜像标签和当前容器的不可变 image ID，再显式传入 SSH 构建会话；不依赖 OpenSSH 自动继承 Docker 环境变量。修改 Compose 或重新构建镜像后必须 `--force-recreate`，避免使用旧容器身份。当前 Dockerfile 启用开发用 root 密码，只适用于受信任的回环环境，不得把 SSH 端口直接暴露到局域网或互联网。自动化连接优先使用已授权私钥。
