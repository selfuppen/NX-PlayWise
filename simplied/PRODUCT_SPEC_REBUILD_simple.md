...
- Switch 后台 组件：随 Atmosphere boot2 启动，通过某种方式，读取或写入 PCTL 家长控制 play timer。
- Switch 前台companion NRO 组件：给孩子提供离线码输入入口，给家长提供密码保护的本地时间管理界面。
...
- 可恢复：通过 `disable.flag` 和 boot2 flag 控制 后台组件 是否影响系统。
