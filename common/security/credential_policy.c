...
{
    size_t used = 0U;
    if (!out || out_size == 0U || !ptc_device_id_valid(device_id) || !ptc_grant_secret_valid(grant_secret)) {
        return false;
    }
    out[0] = '\0';
    return append_text(out, out_size, &used, PTC_PAIRING_BASE_URL "#device_id=") &&
