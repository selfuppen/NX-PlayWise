...
手柄操作为 `L/R` 切页、方向键选择、`A` 执行、`B` 返回、`Y` 刷新。触摸操作与手柄等价：点按标签页切页、点按卡片直接执行、点按弹窗内的确认和取消、点按分钟步进器的加减、七日编辑器的星期与模式、就寝时间字段与增减、限制方式的三个选项。绘制与命中测试共用同一套控件几何，点击区域与看到的控件一致。分钟数使用步进器，每周计划使用七日编辑器，就寝时间使用启用开关和 15 分钟步进，限制方式使用“仅提醒、强制阻止、暂停软件”三段选择器。PIN 使用系统密码软键盘；今日禁玩、快速设备测试、紧急停用和恢复控制通过图形确认弹窗执行，其中后三项会明确展示安全影响。
