...
第六阶段的自动恢复成功后，会话保持 `awaiting_observation`：恢复请求不能跳过或清除待提交的人工观察，Overlay 在此状态隐藏立即恢复入口，NRO 只引导操作者返回浮窗。只有六个自动阶段均存在、人工三态观察已提交并且 `exact_restore_proved` 时，报告的 `summary.complete` 才为 true。
