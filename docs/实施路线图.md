...
| Sysmodule | 部分完成 | 已有 host-testable `sysmodule_core`：queue scanner、stuck recovery、status/offline_code、backup gate、nonce ledger、result 写入；远程 `make` 可作为默认 C host 构建测试入口。 | boot lifecycle、service init、event logger、sysmodule 侧真实 SDMC adapter、observe boot2 binary 和真机启动验证仍需补充。 |
