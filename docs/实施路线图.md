...
| Companion NRO | 部分完成 / 受阻 | 已有 `companion/request_client.c`、`companion/file_protocol.c`、`platform/switch/fs_storage.c` 和 `companion/nro/` 最小 console UI，可提交 `status`/`offline_code` 并轮询 result。 | 需要 devkitPro/libnx 构建和模拟器/真机验证；家长区仍未实现。 |
| 模拟器测试 | 未开始 / 受阻 | 已有详细步骤文档和 NRO 构建目标。 | 需要构建 `build/switch/pctc.nro` 后跑模拟器验证。 |
| 真机 observe | 未开始 / 受阻 | 已有详细步骤文档；可以生成安全 SDMC observe 布局和 safe NRO-only package。 | 需要 sysmodule boot2 skeleton、PCTL read-only adapter，并需要真机验证。 |
...
```sh
ssh 249-nintendo-switch-dev 'cd /ws/switch-play-time-control-local && git pull --ff-only origin master && make && make companion-nro'
```
...
python3 tools/package_sdmc.py --mode safe --out build/packages/safe --zip build/packages/safe.zip
```
...
| Safe NRO-only package | 部分完成 / 受阻 | `make package-safe-nro` 可在 devkitPro 构建 NRO 后生成 zip；仍需模拟器/真机验证。 |
