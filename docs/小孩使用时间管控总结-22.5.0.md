...
不传 `--date` 或 `--day-index` 时，脚本默认生成今天有效的离线码。测试时需要固定输出，可以显式指定日期和 nonce：
...
当前实现使用 `auth.json` 保存本项目 Companion 家长区 PIN 的 salt + HMAC-SHA256。早期草案中的 `settings.conf` 默认 `1234` 明文密码已不再作为当前实现方式。
