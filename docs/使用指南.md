...
普通升级不要使用 `-Clean` 或 `-Full`，这两个选项会清空普通 PlayWise 数据。自制程序菜单高级入口的恢复材料位于原配置旁，不在 `switch/playwise` 内；安装脚本不会处理这些 `.playwise.*` 文件。macOS 或 Linux 目前没有等价的安全升级脚本；请先完整备份 SD 卡中的 `switch/playwise`，再手动升级。
