...
家长区五个顶级页面为“今日额度、周计划、国家节假日、加时码、设置”。顶级页面底栏保留上一页、下一页、返回孩子页和刷新按钮；右侧全局状态栏延伸到主内容右边框，提供完整的当前状态摘要。
...
- **设置**：集中提供外观主题、修改 PIN、家长区快捷键、自制程序菜单入口保护和“支持与恢复”入口。外观主题可选跟随系统、浅色或 OLED Hybrid 暗色，选择后立即生效并保存；暗色只改变主机应用绘制，不改变游戏内浮窗、计时或后台控制。
- **设置 → 支持与恢复**：作为完整二级页面处理接管、保护模式、紧急停用、诊断导出和恢复操作。存在紧急停用、恢复事务或保护故障时，“设置”和该入口会显示“需处理”，家长区也会直接引导到这里；按 `B` 返回设置，二级页内不会用 `L/R` 切换顶级页面。

> [!WARNING]
> 开启自制程序菜单入口保护后，卸载 PlayWise、删除 `switch/playwise` 或格式化 SD 卡前必须先在经过 PIN 认证的“设置”页面关闭保护。该功能保护的是进入自制程序菜单的方式，桌面“手柄设置”图标只是启动载体；它不是完整的系统防篡改边界，孩子区不会显示其状态或快捷键。
