...
- **今日总额度**：临时调整今天全天的总额度、增加时间、设为不限时，或恢复周计划；打开“设置今日总额度”时，已玩时间可靠则默认使用当前可靠总额度与已玩时间中的较大值，已玩时间不可用则沿用可靠的当前限时额度或回退到 60 分钟。“设置今日总额度”和“临时加时”共用同一紧凑编辑器：左侧是数字键盘和快捷增减，右侧分别输入小时、分钟并实时显示总分钟数，同时展示今天已玩、今天还可玩和修改后预计还可玩；触摸可直接选择字段，手柄 Minus 切换字段。今日总额度可选 1–1440 分钟，临时加时默认 15 分钟、可选 1–120 分钟；
...
- **设置 → 高级设置**：页面顶部显示“设置 / 高级设置”和“二级页面”层级提示，可按 `B` 或触摸“返回设置”回到设置页；其中提供可选的“自制程序菜单高级入口”。该功能只改变 hbmenu 启动方式，不是系统防篡改能力。
...
- **家长控制不生效的解决经历（能倒计时且到时间弹出禁用对话框）**：
  部分用户反馈可能会遇到家长控制怎么设置都不生效的情况。有用户经历了：先降级不行(19.x, 20.x, 21.x, 22.0)，然后又再升级到固件22.5，还是不行。最后在22.5，用了整合包里面的时间同步工具之后，莫名其妙的又可以了。
  如果遇到类似问题，你可以尝试用该整合包里的时间同步功能走一遍试试：
  - Tesla 时间同步工具：[https://github.com/ppkantorski/QuickNTP](https://github.com/ppkantorski/QuickNTP)
  - 整合包：[https://docs.qq.com/doc/DVW9PVE5sU0FEd0tP](https://docs.qq.com/doc/DVW9PVE5sU0FEd0tP)
