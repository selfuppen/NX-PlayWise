...
- **今日总额度**：临时调整今天全天的总额度、增加时间、设为不限时，或恢复周计划；这里输入的是包含今天已玩时间的全天总额度，不是“从现在起还能玩”的分钟数。打开“设置今日总额度”时，已玩时间可靠则默认使用当前可靠总额度与已玩时间中的较大值，已玩时间不可用则沿用可靠的当前限时额度或回退到 60 分钟；这个 60 分钟只是编辑初始值，不能保证修改后仍可玩 60 分钟。今日不限时、已玩时间未知或新额度可能已经耗尽时，应用会说明可能立即进入时间限制，并要求长按确认；若设置后受限，可回到今日总额度页选择“今日不限时”或“临时加时”，孩子也可兑换有效加时码。“设置今日总额度”和“临时加时”共用同一紧凑编辑器：左侧是数字键盘和快捷增减，右侧分别输入小时、分钟并实时显示总分钟数，同时展示今天已玩、今天还可玩和修改后预计还可玩；触摸可直接选择字段，手柄 Minus 切换字段。今日总额度可选 1–1440 分钟，临时加时默认 15 分钟、可选 1–120 分钟；
