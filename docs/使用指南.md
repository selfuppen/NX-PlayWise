...
- **今日总额度**：临时调整今天全天的总额度、增加时间、设为不限时，或恢复周计划；打开“设置今日总额度”时默认使用当前已玩时间，已玩 0 分钟时从 1 分钟开始，已玩时间不可用时沿用可靠的当前限时额度或回退到 60 分钟。“设置今日总额度”和“临时加时”共用同一紧凑编辑器：左侧是数字键盘和快捷增减，右侧分别输入小时、分钟并实时显示总分钟数，同时展示今天已玩、今天还可玩和修改后预计还可玩；触摸可直接选择字段，手柄 Minus 切换字段。今日总额度可选 1–1440 分钟，临时加时默认 15 分钟、可选 1–120 分钟；
